...
| `<leader>o` | カーソル下のディレクトリ配下を再帰的に全展開 |
| `<leader>c` | カーソル下 entry の親ディレクトリを折りたたみ |
...
| `q` | etoile を閉じる |
| `<leader>i` | `tree.exclude` で非表示にされた entry の表示/非表示を切り替え |
| `<C-o>` / `<C-i>` | parent/child の root 履歴を backward/forward |
...
`confirm.delete = true` の場合、削除前に確認ウィンドウを表示する。`confirm.move = true` の場合、移動/リネーム前に変更前後のパスを確認ウィンドウに表示する。確認が必要な操作が1つでもある場合、確認ウィンドウには、その操作自体の確認設定が無効なものも含めて、保存で適用されるすべての保留中操作を表示する。確認を cancel しても編集中の tree buffer はそのまま残る。保留中の編集を破棄するには確認ウィンドウで `r` を押す。
...
[lualine.nvim](https://github.com/nvim-lualine/lualine.nvim) が利用可能な場合、preview window にフォーカスした後に preview window 用の lualine を更新する。tree window から preview を開いたり同期したりするだけでは、この更新は行わない。
...
`toggle_exclude` は `tree.exclude` で非表示にされた entry の表示/非表示を切り替える。トグル ON で非表示 entry も tree に表示される（アイコンなし）。トグル OFF で再び非表示にする。
...
`open_split`、`open_vsplit`、`open_tab` は選択中の file をそれぞれ `:split`、`:vsplit`、`:tabedit` で開く。`open_tab_keep` は etoile を閉じずに選択中の file を `:tabedit` で開く。

`expand_all` はカーソル下の directory 配下を再帰的に全展開する。`collapse_parent` はカーソル下 entry の親 directory を折りたたみ、カーソルをその親 directory に移動する。親が tree root で表示行がない top-level entry では何もしない。

`root_history_back` と `root_history_forward` は parent/child の root 履歴を移動する。Etoile の tree buffer では、これらの操作に Neovim の通常 jumplist 移動は使わない。

### search

`search.exclude` は検索時にマッチした entry をスキップする。固定名に加え、`*.log` や `build` などのグロブパターンも指定できる。`search.exclude` にマッチした entry は tree には表示されるが、git status がない場合は左ガターに `󰈉` アイコン（`icons.search_excluded` でカスタマイズ可）が表示される。
...
確認が必要な操作が1つでもある場合、確認ウィンドウには、その操作自体の確認設定が無効なものも含めて、保存で適用されるすべての保留中操作を表示する。Cancel は確認ウィンドウを閉じるだけで、編集中の tree buffer は変更しない。Revert は現在のファイルシステム状態から tree を再描画し、保留中の tree 編集を破棄する。
...
`icons.search_excluded` で `search.exclude` にマッチした entry の左ガターアイコンをカスタマイズできる。git status がない場合に表示される。
