...
`confirm.delete = true` の場合、削除前に確認ウィンドウを表示する。
...
設定可能な mapping はすべて etoile buffer local。`focus_preview` は preview buffer にも mapping されるため、同じキーで main tree へ focus を戻せる。
...
`confirm.delete = true` はファイルやディレクトリの削除前に確認する。`false` にすると、etoile buffer 保存時に削除を即時適用する。
...
TODO: GitHub リポジトリ公開前に `LICENSE` ファイルを追加する。
